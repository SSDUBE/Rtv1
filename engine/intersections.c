...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/11/03 18:36:53 by sdube             #+#    #+#             */
/*   Updated: 2016/11/03 18:36:55 by sdube            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_bool	cylinder_intersection(t_ray *r, t_object *s, t_f64 *t0, t_f64 *t1)
{
	...
	if (!solve_quadratic(dot_vec3f(&dir, &dir), b, c, t0, t1))
		return (FALSE);
	b = s->pos.y;
	c = s->pos.y > 0 ? (s->pos.y + 6) : (6);
	if (!((tray.origin.y + tray.dir.y * *t0) > s->pos.y &&
		(tray.origin.y + tray.dir.y * *t1) < c))
	{
		s->pos.y = c;
		c = sphere_intersection(&tray, s, t0, t1);
		return ((t_bool)c * ((s->pos.y = b) == 0 || 1));
	}
	return (TRUE);
}
